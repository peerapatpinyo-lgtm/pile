...
def calculate_pile_deviation(pu, mx_ext, my_ext, piles_df):
    ...
    Inputs are design coordinates and deviations.
    """
...
st.markdown("Calculate individual pile reactions by inputting **Design Coordinates** and **Deviations** ($\Delta x, \Delta y$).")
...
st.caption("💡 Input the Design Coordinates and the Deviation (offset) for each pile. The system will compute the actual position automatically. All units are in **Meters (m)**.")

# Default Data with Deviations instead of Actual Coordinates
default_data = pd.DataFrame({
    'Pile_Name': ['P1', 'P2', 'P3', 'P4'],
    'x_design': [-0.50, 0.50, -0.50, 0.50],
    'y_design': [0.50, 0.50, -0.50, -0.50],
    'dev_x': [0.02, 0.03, 0.00, 0.01],  # e.g., 2 cm to the right
    'dev_y': [0.05, -0.02, -0.02, 0.05] # e.g., 5 cm up
})
...
if st.button("🧮 Calculate & Generate Report", type="primary"):
    ...
    if df_res is not None:
        ...
        with st.expander("📝 View Step-by-Step Calculation Sheet", expanded=True):
            ...
            # Show actual coordinates calculated
        ...
        with out_col1:
            ...
            df_display = df_res[['Pile_Name', 'dev_x', 'dev_y', 'x_actual', 'y_actual', 'Ri']]
